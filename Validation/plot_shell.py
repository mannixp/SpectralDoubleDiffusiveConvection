...
if __name__ == "__main__":
    ...
    f = h5py.File('snapshots/snapshots_s1.h5', mode='r')
    sim_time = f['tasks/b(phi=0)'].dims[0][0][()]
    phi      = f['tasks/b(phi=0)'].dims[1][0]
    theta    = f['tasks/b(phi=0)'].dims[2][0]
    radius   = f['tasks/b(phi=0)'].dims[3][0]

    print('Sim time (s)',sim_time)

    b = f['tasks/b(phi=0)'][-1,0,:,:]
    plt.pcolor(theta,radius,b.T)
    plt.savefig('b(r,theta).png')
    
    f = h5py.File('scalar_data/scalar_data_s1.h5', mode='r')
    print(f['tasks'].keys())
    fig, (ax1,ax2) = plt.subplots(ncols=2,figsize=(8,4))

    ax1.semilogy(f['scales/sim_time'][()],f['tasks/(1/2)<u^2>'][:,0,0,0] ,'k')
    ax1.set_ylabel(r'Kinetic Energy')
    ax1.set_xlabel(r'Time t')

    ax2.semilogy(f['scales/sim_time'][()],f['tasks/Nu'][:,0,0,0] ,'r')
    #ax1.semilogy(f['scales/sim_time'][()],f['tasks/Nu_o'][:,0,0,0] ,'b')
    ax2.set_ylabel(r'Nusselt Nu')
    ax2.set_xlabel(r'Time t')
    plt.tight_layout()
    
    fig.savefig('Kinetic_Energy.png', dpi=200)
    #fig.clear()
    #plt.close(fig)
    #plt.show()

    nu = f['tasks/Nu'][-1,0,0,0] -1;
    ke = f['tasks/(1/2)<u^2>'][-1,0,0,0]
    print('Nu - 1 = %e'%nu)
    print('KE     = ',ke)