...
!f2py -c --fcompiler=intelem --opt='-O3' --f90flags='-fast -i8 -mkl=parallel' -liomp5 -lpthread -lm -ldl F2PY_LIN_MULT.f90

! ~~~~ f2py -c --fcompiler=intelem --opt='-O3' --f90flags='-fopenmp' -llapack -liomp5 F2PY_LIN_MULT.f90 -m L_PARA ~~~~ To compile Parrallel, can change openmp for -parallel

!adding --f90flags='-fast' compile flag works well
! ~~~~ f2py -c --fcompiler=intelem --opt='-O3' -llapack F2PY_LIN_MULT.f90 -m L_PARA ~~~~ Serial

!~~~~GNU95 ~~~~~~~~~ has problem with fractions
! ~~~~ f2py -c --fcompiler=gnu95 --opt='-O3' --f90flags='-fopenmp' -lgomp F2PY_Module_N1_TSTEP_PARA.f90 -m L_PARA ~~~~ To compile
...
!# ~~~~~~~~~~~~~~~~~~~# #~~~~~~~~~~~~~~~~~~~ #~~~~~~~~~~~~~~~~~~~~~~~~~~~
! ***** Solves Ax = B using dgesv, requires additional flags
!# ~~~~~~~~~~~~~~~~~~~# #~~~~~~~~~~~~~~~~~~~ #~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
function Ax(A,B,N)
    ...
    real(kind=8) :: Ax(size(B));    

    integer :: NRHS,LDA,LDB,INFO
    real(kind=8), dimension(size(A,1),size(A,2)) :: Atemp
    real(kind=8), dimension(size(B)) :: Btemp
    integer, dimension(size(B)) :: IPIV
	
    !set dimensions of A,B
    NRHS = 1; LDA = N; LDB = N

    !allocate arrays, create matrices A_temp, B_temp, IPVN
    !allocate(IPIV(N)); !allocate(Atemp(size(A,1),size(A,2)),Btemp(LDB,NRHS))
...
! ***** Solves -Nabla^2*f = g
subroutine nab2(N_fm,nr,N2R,I,g,f)
 implicit none
 integer, intent(in) :: N_fm,nr
 real(kind=8), dimension(:,:), intent(in) :: N2R,I
 real(kind=8), dimension(:), intent(in) :: g
 real(kind=8), dimension(nr*N_fm), intent(out) :: f
...
! ***** Solves -Pr*A^4*f = g
subroutine A4_BSub(N_fm,nr,IR4,D2,D4,g,f)
 implicit none
 integer, intent(in) :: N_fm,nr
 real(kind=8), dimension(:,:), intent(in) :: IR4,D2,D4
 real(kind=8), dimension(:), intent(in) :: g
 real(kind=8), dimension(nr*N_fm), intent(out) :: f
...
! ***** Solves -Pr*A^4*f = g
!# ~~~~~~~~~~~~~~~~~~~# #~~~~~~~~~~~~~~~~~~~ #~~~~~~~~~~~~~~~~~~~~~~~~~~~
subroutine A4_BSub_SYM(N_fm,nr,IR4,D2,D4,g,f)
 implicit none
 integer, intent(in) :: N_fm,nr
 real(kind=8), dimension(:,:), intent(in) :: IR4,D2,D4
 real(kind=8), dimension(:), intent(in) :: g
...
end subroutine A4_BSub_SYM



! ***** Solves -Nabla^2*f = g
subroutine nab2_SYM(N_fm,nr,N2R,I,g,f)
 implicit none
 integer, intent(in) :: N_fm,nr
 real(kind=8), dimension(:,:), intent(in) :: N2R,I
 real(kind=8), dimension(:), intent(in) :: g
...
subroutine PRECOND_MFREE_SYM(N_fm,nr,Tau, Ra,Ra_s,GX,DT0, N2_INV, A4_INV,D2,IR4,NX,X,AX)
 ...
 real(kind=8),dimension(3*nr*N_fm),intent(out)::AX;
 integer :: N; !jj
 ...
 ! 1) Vorticity
 call a4_bsub_sym_v2(N_fm,nr,A4_INV,D2,IR4,FX(1:N), AX(1:N) )

 ! 2) Temperature
 call nab2_sym_v2(N_fm,nr,N2_INV,FX(1+N:2*N), AX(1+N:2*N) );

 !3) Concentration 
 call nab2_sym_v2(N_fm,nr,N2_INV,FX(1+ 2*N:3*N),AX(1+ 2*N:3*N) );

 AX(1:N) = AX(1:N) - X(1:N);
 AX(1+N:2*N) = AX(1+N:2*N) - X(1+N:2*N);
 AX(1+2*N:3*N) = (1./Tau)*AX(1+2*N:3*N) - X(1+2*N:3*N);
